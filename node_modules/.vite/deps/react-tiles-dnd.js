  ...
  function next() {
    while (env.stack.length) {
      ...
      try {
        ...
        if (rec.async) return Promise.resolve(result2).then(next, function(e2) {
          fail(e2);
          return next();
        });
      } catch (e2) {
...
var require_fast_shallow_equal = __commonJS({
  "../node_modules/fast-shallow-equal/index.js"(exports2) {
    ...
    exports2.equal = function equal(a, b) {
      if (a === b) return true;
      if (!(a instanceof Object) || !(b instanceof Object)) return false;
      var keys2 = keyList(a);
      var length = keys2.length;
      for (var i = 0; i < length; i++)
        if (!(keys2[i] in b)) return false;
      for (var i = 0; i < length; i++)
        if (a[keys2[i]] !== b[keys2[i]]) return false;
...
// ../node_modules/react-tiles-dnd/esm/tile-layout/TilesContainer.js
init_tslib_es6();
var import_react104 = __toESM(require_react());

// ../node_modules/react-tiles-dnd/esm/tile-layout/useTileTable.js
init_tslib_es6();
...
function rubberband(distance, dimension, constant2) {
  if (dimension === 0 || Math.abs(dimension) === Infinity) return Math.pow(distance, constant2 * 5);
...
function rubberbandIfOutOfBounds(position, min2, max2, constant2 = 0.15) {
  if (constant2 === 0) return clamp(position, min2, max2);
  if (position < min2) return -rubberband(min2 - position, max2 - min2, constant2) + min2;
  if (position > max2) return +rubberband(position - max2, max2 - min2, constant2) + max2;
  return position;
}
...
function resolveWith(config = {}, resolvers) {
  ...
  for (const [key, resolver] of Object.entries(resolvers)) {
    switch (typeof resolver) {
      case "function":
        if (true) {
          const r = resolver.call(result2, config[key], key, config);
          if (!Number.isNaN(r)) result2[key] = r;
        } else {
          result2[key] = resolver.call(result2, config[key], key, config);
        }
        break;
      case "object":
        result2[key] = resolveWith(config[key], resolver);
        break;
      case "boolean":
        if (resolver) result2[key] = config[key];
        break;
    }
  }
  return result2;
...
var bindToProps = (props, eventOptions, withPassiveOption) => (device, action, handler, options = {}, isNative2 = false) => {
  var _options$capture, _options$passive;
  const capture = (_options$capture = options.capture) !== null && _options$capture !== void 0 ? _options$capture : eventOptions.capture;
  const passive = (_options$passive = options.passive) !== null && _options$passive !== void 0 ? _options$passive : eventOptions.passive;
  let handlerProp = isNative2 ? device : toHandlerProp(device, action, capture);
...
// ../node_modules/react-tiles-dnd/esm/tile-layout/table-utils/tiles-table.js
init_tslib_es6();

// ../node_modules/react-tiles-dnd/esm/tile-layout/table-utils/table.js
init_tslib_es6();
var newTable = function(rows, cols, value) {
  return new Array(rows).fill(null).map(function(_) {
    return new Array(cols).fill(value);
  });
};
var copyTable = function(table) {
  return __spreadArray([], __read(table)).map(function(row) {
    return __spreadArray([], __read(row));
  });
};
...
var placeInTable = function(coords, data, table) {
  var newTable2 = copyTable(table);
  for (var row = coords.row; row < coords.row + coords.rowSpan; row++) {
    for (var col = coords.col; col < coords.col + coords.colSpan; col++) {
      newTable2[row][col] = data;
    }
  }
  return newTable2;
};
...
var interceptTiles = function(table, startRow, startCol, rowSpan, colSpan) {
  ...
  for (var row = startRow; row < startRow + rowSpan; row++) {
    ...
    var _loop_1 = function(col2) {
      if (col2 >= cols)
        return "continue";
      var cell = table[row][col2];
      if (cell && !interceptTiles2.find(function(t) {
        return t.data === cell.data;
      })) {
        interceptTiles2.push(cell);
      }
    };
  ...
  return interceptTiles2.sort(function(a, b) {
    return a.col + a.row * cols - (b.col + b.row * cols);
  });
};
...
var tilesListToTable = function(tiles, columns) {
  var maxRows = tiles.reduce(function(memo, tile) {
    return memo + tile.rowSpan;
  }, 0);
  var table = newTable(maxRows, columns, void 0);
...
// ../node_modules/react-tiles-dnd/esm/tile-layout/strategies/move.js
init_tslib_es6();

// ../node_modules/react-tiles-dnd/esm/tile-layout/table-utils/tiles.js
init_tslib_es6();
var getTileTouchPoint = function(draggingTile, targetTile, absolutePosition, _a) {
  ...
  if (draggingTile.col >= targetTile.col && position.x < activeBorderSize)
    return "left";
  if (draggingTile.col <= targetTile.col && position.x > elementWidth * targetTile.colSpan - activeBorderSize)
    return "right";
  if (draggingTile.row <= targetTile.row && position.y > elementHeight * targetTile.rowSpan - activeBorderSize)
    return "bottom";
  if (draggingTile.row >= targetTile.row && position.y < activeBorderSize)
    return "top";
  if (position.x > activeBorderSize && position.y < elementWidth * targetTile.colSpan - activeBorderSize && position.y > activeBorderSize && position.y < elementHeight * targetTile.rowSpan - activeBorderSize)
    return "center";
};
...
var dragMove = {
  onDragMove: function(_a) {
    ...
    if (cell.col <= tsize.cols && cell.row <= tsize.rows) {
      ...
      var hoverTiles_1 = interceptTiles(table, newDragTileLocation.row, newDragTileLocation.col, draggingTile.rowSpan, draggingTile.colSpan).filter(function(tiles2) {
        return tiles2.data !== draggingTile.data;
      });
      if (!hoverTiles_1.length) {
        return;
      }
      var otherTiles = tiles.filter(function(tile) {
        return tile.data !== draggingTile.data && !hoverTiles_1.find(function(t) {
          return t.data === tile.data;
        });
      });
      ...
      var reorderedTiles = __spreadArray(__spreadArray(__spreadArray([], __read(repositionedHoverTiles_1)), __read(otherTiles)), [
        ...
        return a.col + a.row * columns - (b.col + b.row * columns);
      });
      return {
...
var move_default = dragMove;

// ../node_modules/react-tiles-dnd/esm/tile-layout/strategies/reorder.js
var reorder = {
  onDragMove: function(_a) {
    var _b;
    var offsetX = _a.offsetX, offsetY = _a.offsetY, config = _a.config, state = _a.state, table = _a.table, canAcceptDrop = _a.canAcceptDrop;
    ...
    if (!draggingTile || !dragPosition || !start || !tiles)
      return;
    var getInsertionPoint = function(tiles2, touchPoint2, touchedTile2) {
      var tilePosition = tiles2.findIndex(function(tile) {
        return tile.data === (touchedTile2 === null || touchedTile2 === void 0 ? void 0 : touchedTile2.data);
      });
      if (touchPoint2 === "left") {
        var leftTile = tilePosition - 1 >= 0 ? tiles2[tilePosition - 1] : void 0;
        if (leftTile === draggingTile)
          return void 0;
        return {
          insertionPoint: {
            right: touchedTile2,
            left: (leftTile === null || leftTile === void 0 ? void 0 : leftTile.row) === touchedTile2.row ? leftTile : void 0
          }
        };
      } else {
        var rightTile = tilePosition + 1 < tiles2.length ? tiles2[tilePosition + 1] : void 0;
        if (rightTile === draggingTile)
          return void 0;
        return {
          insertionPoint: {
            left: touchedTile2,
            right: (rightTile === null || rightTile === void 0 ? void 0 : rightTile.row) === touchedTile2.row ? rightTile : void 0
          }
        };
      }
    };
    ...
    if (touchedTile.row !== cell.row) {
      ...
      return {
        insertionPoint: {
          left: touchPoint === "left" ? shiftedInsertionPoint === null || shiftedInsertionPoint === void 0 ? void 0 : shiftedInsertionPoint.right : void 0,
          right: touchPoint === "right" ? shiftedInsertionPoint === null || shiftedInsertionPoint === void 0 ? void 0 : shiftedInsertionPoint.left : void 0
        }
      };
    }
  ...
  onDragEnd: function(_a) {
    ...
    if (!tiles || !draggingTile || !insertionPoint)
      return;
    var newTiles = tiles.filter(function(tile) {
      return tile.data !== draggingTile.data;
    });
    var left = insertionPoint.left, right = insertionPoint.right;
    var insertionLeftIdx = left ? newTiles.findIndex(function(tile) {
      return tile.data === left.data;
    }) + 1 : -1;
    var insertionRightIdx = right ? newTiles.findIndex(function(tile) {
      return tile.data === right.data;
    }) : -1;
...
var useTileTable = function(_a) {
  ...
  var strategyImpl = strategy === "reorder" ? reorder_default : move_default;
  var _b = __read((0, import_react2.useState)({
    dragging: false,
    droppable: false
  ...
  var table = (0, import_react2.useMemo)(function() {
    return tilesListToTable(effectiveTiles, columns);
  }, [effectiveTiles, columns]);
  var positionedTiles = (0, import_react2.useMemo)(function() {
    return tableToTilesList(table);
  }, [table]);
  var config = {
    elementHeight,
    elementWidth,
  ...
  var bind2 = useDrag(function(_a2) {
    var _b2 = __read(_a2.args, 1), data = _b2[0], dragging2 = _a2.dragging, tap2 = _a2.tap, xy = _a2.xy, movement = _a2.movement, event = _a2.event;
    if (dragging2) {
      if (!tap2) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (!state.dragging) {
        ...
        var draggingTile_1 = positionedTiles.find(function(tile) {
        ...
        setState(function(state2) {
          return __assign(__assign(__assign({}, state2), { offset: { x: movement[0], y: movement[1] }, dropTargetTile: void 0, droppable: false, insertionPoint: void 0 }), result_1);
        });
      }
    } else {
      if (state.draggingTile && state.dropTargetTile && state.droppable) {
        ...
        var result2 = strategyImpl.onDragEnd({
          offsetX: movement[0],
          offsetY: movement[1],
          canAcceptDrop,
          config,
        ...
        var finalState = __assign(__assign({}, state), result2);
...
var useSetState = function(initialState) {
  ...
  var _a = (0, import_react14.useState)(initialState), state = _a[0], set2 = _a[1];
  var setState = (0, import_react14.useCallback)(function(patch) {
...
function createHTMLMediaHook(tag) {
  return function(elOrProps) {
    var element;
    var props;
    if (React2.isValidElement(elOrProps)) {
      element = elOrProps;
      props = element.props;
    } else {
      props = elOrProps;
...
var useIsomorphicLayoutEffect = isBrowser2 ? import_react26.useLayoutEffect : import_react26.useEffect;
...
var isIosDevice = isBrowser2 && window.navigator && window.navigator.platform && /iP(ad|hone|od)/.test(window.navigator.platform);
...
var useMediaDevices = function() {
  var _a = (0, import_react59.useState)({}), state = _a[0], setState = _a[1];
...
function useMeasure() {
  var _a = (0, import_react97.useState)(null), element = _a[0], ref = _a[1];
  var _b = (0, import_react97.useState)(defaultState), rect = _b[0], setRect = _b[1];
  var observer = (0, import_react97.useMemo)(function() {
...
var useMeasure_default = isBrowser2 && typeof window.ResizeObserver !== "undefined" ? useMeasure : function() {
...
// ../node_modules/react-tiles-dnd/esm/tile-layout/TilesContainer.module.css.js
...
var TilesContainer_module_css_default = styles;

// ../node_modules/react-tiles-dnd/esm/utils/useComposedRef.js
...
var useComposeRef = function() {
  ...
  return (0, import_react103.useCallback)(function(el) {
...
// ../node_modules/react-tiles-dnd/esm/utils/joinclasses.js
var jc = function() {
  var s = [];
  for (var _i = 0; _i < arguments.length; _i++) {
    s[_i] = arguments[_i];
...
function createHybrid(func, bitmask, thisArg, partials, holders, partialsRight, holdersRight, argPos, ary2, arity) {
  ...
  function wrapper() {
    ...
    length = args.length;
    if (argPos) {
      args = reorder_default2(args, argPos);
...
function mixin(object, source, options) {
  ...
  var chain3 = !(isObject_default(options) && "chain" in options) || !!options.chain, isFunc = isFunction_default(object);
...
// ../node_modules/react-tiles-dnd/esm/tile-layout/TilesContainer.js
var TileUI = import_react104.default.memo(function(_a) {
  var renderTile = _a.renderTile, props = __rest(_a, ["renderTile"]);
  return renderTile(props);
});
var isTilesContainerForcedSizeProps = function(props) {
  return !!props.forceTileWidth;
};
var isTilesContainerColsProps = function(props) {
  return !!props.columns;
};
var defaultTileSize = function() {
  return { rowSpan: 1, colSpan: 1 };
};
var lastKey = 0;
var dataWithKeys = function(data, dataWithKeys2) {
  return data.map(function(item) {
    var existingItem = dataWithKeys2.find(function(k) {
      return k.item === item;
    });
...
var TilesContainer = function(props) {
  ...
  var _a = __read(useMeasure_default(), 2), measureRef = _a[0], measure = _a[1];
  var forceTileWidth = isTilesContainerForcedSizeProps(props) ? props.forceTileWidth : 0;
  var columns = isTilesContainerColsProps(props) ? props.columns : forceTileWidth ? Math.floor(measure.width / forceTileWidth) : 4;
  ...
  var _f = __read((0, import_react104.useState)(function() {
    return dataWithKeys(propsData, []);
  }), 2), data = _f[0], setData2 = _f[1];
  (0, import_react104.useEffect)(function() {
    return setData2(function(curr) {
      return dataWithKeys(propsData, curr);
    });
  }, [propsData, setData2]);
  var propsTiles = (0, import_react104.useMemo)(function() {
    var tiles2 = [];
    data.forEach(function(_a2) {
      var tile = _a2.item, key = _a2.key;
      return tiles2.push(__assign(__assign({ key }, tileSize(tile)), { data: tile }));
    });
    return tiles2;
  }, [data, tileSize]);
  var _g = useTileTable({
    ...
    changeTilesOrder: function(tiles2) {
      var newData = tiles2.map(function(tile) {
        return tile.data;
      });
      var actualData = data.map(function(tile) {
        return tile.item;
      });
      if (!isEqual_default(actualData, newData)) {
        setData2(function(curr) {
          return dataWithKeys(newData, curr);
        });
        onReorderTiles && onReorderTiles(newData);
      }
    },
    didDrop: function(source, target) {
      setData2(function(tiles2) {
        return tiles2.filter(function(tile) {
          return tile.item !== source.data;
        });
      });
      onTileDrop && onTileDrop(source.data, target.data);
    }
  }), bind2 = _g.bind, renderTileProps = _g.renderTileProps, tableHeight = _g.tableHeight, insertIndicatorPosition = _g.insertIndicatorPosition;
  var insertIndicator = (0, import_react104.useMemo)(function() {
    if (!insertIndicatorPosition || !renderInsertIndicator) {
      return null;
    }
    var x = insertIndicatorPosition.x, y = insertIndicatorPosition.y;
    return import_react104.default.createElement("div", { className: TilesContainer_module_css_default.indicator, style: {
      top: y + "px",
      left: x + "px",
  ...
  var tiles = (0, import_react104.useMemo)(function() {
    return renderTileProps.map(function(_a2) {
      var x = _a2.x, y = _a2.y, key = _a2.key, props2 = __rest(_a2, ["x", "y", "key"]);
      return import_react104.default.createElement(
        "div",
        __assign({ className: jc(TilesContainer_module_css_default.tile, props2.isDragging && TilesContainer_module_css_default.dragging), key: "K-" + key, style: {
          top: y + "px",
          left: x + "px",
          width: props2.colSpan * tileWidth + "px",
          height: props2.rowSpan * tileHeight + "px"
        } }, bind2(props2.data)),
        import_react104.default.createElement(TileUI, __assign({}, props2, { renderTile }))
      );
    });
  }, [renderTileProps, renderTile, bind2, tileWidth, tileHeight]);
  return import_react104.default.createElement(
    "div",
    { className: TilesContainer_module_css_default.container, ref: useComposeRef(containerRef, measureRef), style: { height: tableHeight + "px", minWidth: tileWidth + "px" } },
    insertIndicator,
    tiles
  );
};
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=react-tiles-dnd.js.map
